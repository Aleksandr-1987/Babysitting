<template>
    ...
    <ul>                
        <li v-for="post in alcohols" :key="post.id" class="category_item">
            {{ post.title }} 
...
import {mapActions, mapState} from 'vuex';
    export default {
        ...
        methods: {
            ...mapActions([
                'GET_ALCOHOLS', 'CREATE_ALCOHOL', 'GET_ALCOHOL', 'DELETE_ALCOHOL'
            ]),
            createAlcohol() {
                this.CREATE_ALCOHOL(this.alcohol);
                this.alcohol.title = '';
            },
            change_alcohol(id, title) {
                this.fix_alcohol.id = id;
                this.fix_alcohol.title = title;
                this.GET_ALCOHOL(this.fix_alcohol);
                this.$router.push({name: "Change-alcohol"})                
            },
            delete_alcohol(id) {                
                this.DELETE_ALCOHOL(id);                                
            } 
        },
        mounted() {
            this.GET_ALCOHOLS();                        
