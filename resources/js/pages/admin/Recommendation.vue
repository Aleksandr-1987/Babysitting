<template>
    ...
    <ul>                
        <li v-for="post in recommendations" :key="post.id" class="category_item">
            {{ post.title }} 
...
import {mapActions, mapState} from 'vuex';
    export default {
        ...
        methods: {
            ...mapActions([
                'GET_RECOMMENDATIONS', 'CREATE_RECOMMENDATION', 'GET_RECOMMENDATION', 'DELETE_RECOMMENDATION'
            ]),
            createRecommendation() {
                this.CREATE_RECOMMENDATION(this.recommendation);
                this.recommendation.title = '';
            },
            change_recommendation(id, title) {
                this.fix_recommendation.id = id;
                this.fix_recommendation.title = title;
                this.GET_RECOMMENDATION(this.fix_recommendation);
                this.$router.push({name: "Change-recommendation"})                
            },
            delete_recommendation(id) {                
                this.DELETE_RECOMMENDATION(id);                                
            } 
        },
        mounted() {
